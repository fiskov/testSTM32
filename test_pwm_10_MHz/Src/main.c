...
void uart_send(char bfr[], uint16_t length)
{
  while (length--)
  {
    while (!LL_USART_IsActiveFlag_TXE(USART1));
    LL_USART_TransmitData8(USART1, (uint8_t)*bfr++);
  }
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  */
  ...
  init_exti();
  LL_TIM_SetAutoReload(TIM3, 3 * TIME_1_us);
  ...
  while (1)
  {
    if (data.flags.timer1s) 
    {      
      data.flags.timer1s = 0;
